...
float pgain( long x, Points *points, float z, long int *numcenters, int kmax, bool *is_center, int *center_table, bool *switch_membership,
							double *serial, double *cpu_gpu_memcpy, double *memcpy_back, double *gpu_malloc, double *kernel,cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{	
	cudaSetDevice(0);
#ifdef PROFILE
	double t1 = gettime();
	...
	*serial += t2 - t1;
	...
	if( c == 0 ) {
		...
		*serial += t5 - t4;
		...
		cudaMemcpyAsync( switch_membership_d,  switch_membership,  num*sizeof(bool),  cudaMemcpyHostToDevice, stream_app);
		cudaMemcpyAsync( coord_d,  coord_h,  num*dim*sizeof(float),  cudaMemcpyHostToDevice, stream_app);
#ifdef PROFILE
		double t6 = gettime();
	...
	cudaMemcpyAsync( center_table_d,  center_table,  num*sizeof(int),  cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync( p,  points->p,  num * sizeof(Point),  cudaMemcpyHostToDevice, stream_app);
	...
	*cpu_gpu_memcpy += t8 - t7;
	...
	pgain_kernel<<< grid_size, THREADS_PER_BLOCK,  smSize, stream_app>>>(	
																											num,								// in:	# of data
	...
	*kernel += t10 - t9;
	...
	cudaMemcpyAsync(work_mem_h, work_mem_d, (K+1) *num*sizeof(float), cudaMemcpyDeviceToHost, stream_app);
	cudaMemcpyAsync(switch_membership, switch_membership_d, num * sizeof(bool), cudaMemcpyDeviceToHost, stream_app);
