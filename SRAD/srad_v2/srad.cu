...
static void runTest( cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag);

////////////////////////////////////////////////////////////////////////////////
// Program main
////////////////////////////////////////////////////////////////////////////////
int
main_SRADV2( cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag) 
{
...
static void
runTest( cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag) 
{
 ...
 for (iter=0; iter< niter; iter++){     
	...
	cudaMemcpyAsync(J_cuda, J, sizeof(float) * size_I, cudaMemcpyHostToDevice, stream_app);

	//Run kernels
	srad_cuda_1<<<dimGrid, dimBlock, 0, stream_app>>>(E_C, W_C, N_C, S_C, J_cuda, C_cuda, cols, rows, q0sqr); 
	srad_cuda_2<<<dimGrid, dimBlock, 0, stream_app>>>(E_C, W_C, N_C, S_C, J_cuda, C_cuda, cols, rows, lambda, q0sqr); 

	pthread_mutex_unlock (mutexapp);

	//Copy data from device memory to main memory
    cudaMemcpyAsync(J, J_cuda, sizeof(float) * size_I, cudaMemcpyDeviceToHost, stream_app);
