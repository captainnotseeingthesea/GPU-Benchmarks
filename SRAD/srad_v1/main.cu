...
int main_SRADV1(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag){
	...
	niter = 10;
	lambda = 0.5;
	Nr = 502;						// it is 502 in the original image
	Nc = 458;						// it is 458 in the original image
	...
	read_graphics(	"SRAD/srad_v1/image.pgm",
								image_ori,
								image_ori_rows,
								image_ori_cols,
								1);
	...
	extract<<<blocks, threads, 0, stream_app>>>(	Ne,
									d_I);
	...
	// fflush(NULL);
		...
		prepare<<<blocks, threads, 0, stream_app>>>(	Ne,
										d_I,
										d_sums,
										d_sums2);
		...
		while(blocks2.x != 0){
			...
			// run kernel
			reduce<<<blocks2, threads, 0, stream_app>>>(	Ne,
											no,
											mul,
											d_sums, 
											d_sums2);
		...
		cudaMemcpyAsync(&total, d_sums, mem_size_single, cudaMemcpyDeviceToHost, stream_app);
		cudaMemcpyAsync(&total2, d_sums2, mem_size_single, cudaMemcpyDeviceToHost, stream_app);
		...
		// execute srad kernel
		srad<<<blocks, threads, 0, stream_app>>>(	lambda,									// SRAD coefficient 
		...
		srad2<<<blocks, threads, 0, stream_app>>>(	lambda,									// SRAD coefficient 
									Nr,										// # of rows in input image
									Nc,										// # of columns in input image
									Ne,										// # of elements in input image
									d_iN,									// indices of North surrounding pixels
	...
	compress<<<blocks, threads, 0, stream_app>>>(	Ne,
									d_I);
