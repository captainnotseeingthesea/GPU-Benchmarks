...
int main_SAD(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
  ...
  ref_image = load_image((char*)"SAD/reference.bin");
  cur_image = load_image((char*) "SAD/frame.bin");
  ...
  /* Run the kernel code */
  {
    ...
    larger_sad_calc_8<<<macroblock_grid, dim3(32, 4),0, stream_app>>>
      (d_sads,
       image_width_macroblocks,
       image_height_macroblocks);
    CUDA_ERRCK

    larger_sad_calc_16<<<macroblock_grid, dim3(32, 1),0,stream_app>>>
      (d_sads,
       image_width_macroblocks,
       image_height_macroblocks);
    CUDA_ERRCK
  ...
  //if (params->outFile)
    //{
      pb_SwitchToTimer(&timers, pb_TimerID_IO);
      write_sads((char*) "SAD/out.bin", image_size_macroblocks, sads_computed);
      pb_SwitchToTimer(&timers, pb_TimerID_COMPUTE);
    //}
