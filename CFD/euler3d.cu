...
template <typename T>
void copy(T* dst, T* src, int N,cudaStream_t stream_app)
{
	CUDA_SAFE_CALL(cudaMemcpyAsync((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToDevice, stream_app));
}

template <typename T>
void upload(T* dst, T* src, int N,cudaStream_t stream_app)
{
	CUDA_SAFE_CALL(cudaMemcpyAsync((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyHostToDevice, stream_app));
	std:: cout << "I am in upload\n";
}

template <typename T>
void download(T* dst, T* src, int N,cudaStream_t stream_app)
{
	CUDA_SAFE_CALL(cudaMemcpyAsync((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToHost, stream_app));
}

void dump(float* variables, int nel, int nelr, cudaStream_t stream_app)
{
	float* h_variables = new float[nelr*NVAR];
	download(h_variables, variables, nelr*NVAR, stream_app);
...
void initialize_variables(int nelr, float* variables, cudaStream_t stream_app)
{
	dim3 Dg(nelr / block_length), Db(block_length);
	std::cout <<"Beginning...\n";
	cuda_initialize_variables<<<Dg, Db,0, stream_app>>>(nelr, variables);
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors, cudaStream_t stream_app)
{
	dim3 Dg(nelr / block_length), Db(block_length);
	cuda_compute_step_factor<<<Dg, Db,0,stream_app>>>(nelr, variables, areas, step_factors);		
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes, cudaStream_t stream_app)
{
	dim3 Dg(nelr / block_length), Db(block_length);
	cuda_compute_flux<<<Dg,Db,0, stream_app>>>(nelr, elements_surrounding_elements, normals, variables, fluxes);
...
void time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes, cudaStream_t stream_app)
{
	dim3 Dg(nelr / block_length), Db(block_length);
	cuda_time_step<<<Dg,Db,0, stream_app>>>(j, nelr, old_variables, variables, step_factors, fluxes);
...
int main_cfd(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	const char* data_file_name = (char*)"CFD/fvcorr.domn.193K";
	...
	// set far field conditions and load them into constant memory on the gpu
	{
		...
		CUDA_SAFE_CALL( cudaMemcpyToSymbolAsync(ff_variable,          h_ff_variable,          NVAR*sizeof(float),0, cudaMemcpyHostToDevice, stream_app) );
		CUDA_SAFE_CALL( cudaMemcpyToSymbolAsync(ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x, sizeof(float3),0, cudaMemcpyHostToDevice, stream_app) );
		CUDA_SAFE_CALL( cudaMemcpyToSymbolAsync(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y, sizeof(float3),0, cudaMemcpyHostToDevice, stream_app) );
		CUDA_SAFE_CALL( cudaMemcpyToSymbolAsync(ff_flux_contribution_momentum_z, &h_ff_flux_contribution_momentum_z, sizeof(float3),0, cudaMemcpyHostToDevice, stream_app) );
		
		CUDA_SAFE_CALL( cudaMemcpyToSymbolAsync(ff_flux_contribution_density_energy, &h_ff_flux_contribution_density_energy, sizeof(float3),0, cudaMemcpyHostToDevice, stream_app) );		
	}
	int nel;
	int nelr;
	...
	float* normals;
	{
		...
		upload<float>(areas, h_areas, nelr, stream_app);

		elements_surrounding_elements = alloc<int>(nelr*NNB);
		upload<int>(elements_surrounding_elements, h_elements_surrounding_elements, nelr*NNB, stream_app);

		normals = alloc<float>(nelr*NDIM*NNB);
		upload<float>(normals, h_normals, nelr*NDIM*NNB, stream_app);
	...
	initialize_variables(nelr, variables,stream_app);
	...
	initialize_variables(nelr, old_variables, stream_app);
	initialize_variables(nelr, fluxes, stream_app);
	pthread_mutex_unlock (mutexapp);
	if(flag)
		cutilSafeCall( cudaStreamSynchronize(stream_app) );
	else
	...
	{	
		copy<float>(old_variables, variables, nelr*NVAR,stream_app);
		
		// for the first iteration we compute the time step
		compute_step_factor(nelr, variables, areas, step_factors, stream_app);
		CUT_CHECK_ERROR("compute_step_factor failed");
		std::cout << "Done with compute step"<< std ::endl;
		pthread_mutex_unlock (mutexapp);
		if(flag)
			cutilSafeCall( cudaStreamSynchronize(stream_app) );
		else
		...
		for(int j = 0; j < RK; j++)
		{
			compute_flux(nelr, elements_surrounding_elements, normals, variables, fluxes, stream_app);
			CUT_CHECK_ERROR("compute_flux failed");	
			std::cout << "Done with compute flux"<< std ::endl;
			pthread_mutex_unlock (mutexapp);
			if(flag)
				cutilSafeCall( cudaStreamSynchronize(stream_app) );
			else
			...
			time_step(j, nelr, old_variables, variables, step_factors, fluxes, stream_app);
			CUT_CHECK_ERROR("time_step failed");
			std::cout << "Done with time step"<< std ::endl;
			pthread_mutex_unlock (mutexapp);
			if(flag)
				cutilSafeCall( cudaStreamSynchronize(stream_app) );
			else
	...
	std::cout << "end of loop heyyo" << std:: endl;
	pthread_mutex_unlock (mutexapp);
	...
	dump(variables, nel, nelr, stream_app);
