...
void computePhiMag_GPU(int numK, float* phiR_d, float* phiI_d, float* phiMag_d, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
  ...
  ComputePhiMag_GPU <<< DimPhiMagGrid, DimPhiMagBlock, 0, stream_app>>> 
...
void computeQ_GPU(int numK, int numX,
                  ...
                  kValues* kVals,
                  float* Qr_d, float* Qi_d, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
  ...
  for (int QGrid = 0; QGrid < QGrids; QGrid++) {
    ...
    cudaMemcpyToSymbolAsync(ck, kValsTile, numElems * sizeof(kValues), 0, cudaMemcpyHostToDevice, stream_app);

    ComputeQ_GPU <<< DimQGrid, DimQBlock, 0, stream_app >>>
