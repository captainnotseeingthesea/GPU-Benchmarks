...
//
// ****************************************************************************
void runTest(const string& name,cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag);
...
int main_QTC(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag){
    ...
    runTest("QTC", stream_app, mutexapp, flag);
	return 0;
}
...
{    
    ...
    QTC(name, matrix_type, stream_app, mutexapp, flag);

}

////////////////////////////////////////////////////////////////////////////////
//
void QTC(const string& name, int matrix_type,cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag){
    ...
    if( can_use_texture ){
        ...
        copyToDevice(distance_matrix_gmem, dist_source, dst_matrix_elems*sizeof(float), stream_app);
    }

    copyToDevice(indr_mtrx, indr_mtrx_host, point_count*max_degree*sizeof(int), stream_app);

    copyToDevice(ungrpd_pnts_indr, ungrpd_pnts_indr_host, point_count*sizeof(int), stream_app);
    ...
    compute_degrees<<<grid2D(thread_block_count), tpb,0,stream_app>>>((int *)indr_mtrx, (int *)degrees, point_count, max_degree);
    ...
    do{
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////
        ///////// -----------------               Main kernel                ----------------- /////////
        QTC_device<<<grid, tpb,0, stream_app>>>((float*)distance_matrix, (char *)Ai_mask, (char *)clustered_pnts_mask,
        ...
        if( thread_block_count > 1 ){
            ...
            reduce_card_device<<<grid2D(1), 1,0, stream_app>>>((int *)cardnl, thread_block_count);
			pthread_mutex_unlock(mutexapp);	
			if(flag)
				cutilSafeCall( cudaStreamSynchronize(stream_app) );
			else
        ...
        copyFromDevice( cardinalities, cardnl, 2*sizeof(int), stream_app );
		...
		pthread_mutex_lock(mutexapp);	
        trim_ungrouped_pnts_indr_array<<<grid2D(1), tpb,0, stream_app>>>(winner_index, (int*)ungrpd_pnts_indr, (float*)distance_matrix,
        ...
        if( cwrank == winner_node){ // for non-parallel cases, these should both be zero.
            ...
            copyFromDevice(output, (void *)result, max_card*sizeof(int) , stream_app);
		...
		pthread_mutex_lock(mutexapp);	
        update_clustered_pnts_mask<<<grid2D(1), tpb,0, stream_app>>>((char *)clustered_pnts_mask, (char *)Ai_mask, max_point_count);
        pthread_mutex_unlock(mutexapp);	
		if(flag)
			cutilSafeCall( cudaStreamSynchronize(stream_app) );
		else
...
void
copyToDevice(void* to_device, void* from_host, unsigned long bytes, cudaStream_t stream_app)
{
    cudaMemcpyAsync(to_device, from_host, bytes, cudaMemcpyHostToDevice, stream_app);
    CHECK_CUDA_ERROR();
}


void
copyFromDevice(void* to_host, void* from_device, unsigned long bytes,cudaStream_t stream_app)
{
    cudaMemcpyAsync(to_host, from_device, bytes, cudaMemcpyDeviceToHost, stream_app);
    CHECK_CUDA_ERROR();
}
