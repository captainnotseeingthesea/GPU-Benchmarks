...
void Cuda_memcpyfromHostToDevice(float *ptr_gpu,float *ptr_cpu,int size_in_bytes,cudaStream_t stream_app)
{
    DEBUGPRINT(("CUDA : Allocate memcpy host to device\n"));
	cudaError_t err = cudaSuccess;
	err = cudaMemcpyAsync(ptr_gpu,ptr_cpu,size_in_bytes,cudaMemcpyHostToDevice, stream_app);// FILL HERE
	if (err != cudaSuccess)
	{
...
void Cuda_memcpyfromDeviceToHost(float *ptr_cpu,float *ptr_gpu,int size_in_bytes,cudaStream_t stream_app)
{
    DEBUGPRINT(("CUDA : Allocate memcpy device to host\n"));
	cudaError_t err = cudaSuccess;
	err = cudaMemcpyAsync(ptr_cpu,ptr_gpu,size_in_bytes,cudaMemcpyDeviceToHost, stream_app);// FILL HERE
	if (err != cudaSuccess)
	{
        DEBUGPRINT(("CUDA Memcpy Failed\n")); 
	}
...
//Keras
	...
	extract_weights("data_GRU/kernel_h.txt",kernel_h_w);
	extract_weights("data_GRU/kernel_r.txt",kernel_r_w);
	extract_weights("data_GRU/kernel_z.txt",kernel_z_w);
	extract_weights("data_GRU/bias_h.txt",bias_h);
	extract_weights("data_GRU/bias_r.txt",bias_r);
	extract_weights("data_GRU/bias_z.txt",bias_z);
	extract_weights("data_GRU/recurrent_kernel_h.txt",rec_h_w);
	extract_weights("data_GRU/recurrent_kernel_r.txt",rec_r_w);
	extract_weights("data_GRU/recurrent_kernel_z.txt",rec_z_w);
	extract_weights("data_GRU/dense_kernel.txt",dense_kernel);
	extract_weights("data_GRU/dense_bias.txt",dense_bias);
	...
	Cuda_memcpyfromHostToDevice(GPU_kernel_h_w,kernel_h_w,sizeof(float) * size_wGates, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_bias_h,bias_h,sizeof(float) * size_bGates, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_rec_h_w,rec_h_w,sizeof(float) * size_wHidden, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_kernel_r_w,kernel_r_w,sizeof(float) * size_wGates, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_bias_r,bias_r,sizeof(float) * size_bGates, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_rec_r_w,rec_r_w,sizeof(float) * size_wHidden, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_kernel_z_w,kernel_z_w,sizeof(float) * size_wGates, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_bias_z,bias_z,sizeof(float) * size_bGates, stream_app);
	Cuda_memcpyfromHostToDevice(GPU_rec_z_w,rec_z_w,sizeof(float) * size_wHidden, stream_app);
	...
	Cuda_memcpyfromHostToDevice(GPU_x,x,sizeof(float) * input_dim, stream_app);
	...
	GPU_forward_pass_gru<<<numBlocks, numThreads, 0, stream_app>>>(GPU_x, GPU_states ,GPU_kernel_h_w,GPU_kernel_r_w,GPU_kernel_z_w,GPU_bias_h,GPU_bias_r,GPU_bias_z,GPU_rec_h_w,GPU_rec_r_w,GPU_rec_z_w);
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	Cuda_memcpyfromDeviceToHost(CPU_states,GPU_states,sizeof(float) * num_units, stream_app);
    prodsum = 0; 
