...
int main_lib(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag){
  ...
  cudaMemcpyToSymbolAsync(N, &h_N, sizeof(h_N),0, cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyToSymbolAsync(Nmat, &h_Nmat, sizeof(h_Nmat),0, cudaMemcpyHostToDevice,stream_app);
  cudaMemcpyToSymbolAsync(Nopt, &h_Nopt, sizeof(h_Nopt),0, cudaMemcpyHostToDevice,stream_app);
  cudaMemcpyToSymbolAsync(delta, &h_delta, sizeof(h_delta),0, cudaMemcpyHostToDevice,stream_app);
  cudaMemcpyToSymbolAsync(maturities, &h_maturities, sizeof(h_maturities),0, cudaMemcpyHostToDevice,stream_app);
  cudaMemcpyToSymbolAsync(swaprates, &h_swaprates, sizeof(h_swaprates),0, cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyToSymbolAsync(lambda, &h_lambda, sizeof(h_lambda),0, cudaMemcpyHostToDevice,stream_app);
...
//pthread_mutex_lock (mutexapp); 
  Pathcalc_Portfolio_KernelGPU2<<<dimGrid, dimBlock,0,stream_app>>>(d_v);
  ...
  CUDA_SAFE_CALL( cudaMemcpyAsync(h_v, d_v, sizeof(float)*NPATH,
...
if(flag)
  ...
  // Launch the device computation threads
  pthread_mutex_lock(mutexapp);
  Pathcalc_Portfolio_KernelGPU<<<dimGrid, dimBlock,0, stream_app>>>(d_v,d_Lb);
  ...
  CUDA_SAFE_CALL( cudaMemcpyAsync(h_v, d_v, sizeof(float)*NPATH,
                  cudaMemcpyDeviceToHost, stream_app) );
  CUDA_SAFE_CALL( cudaMemcpyAsync(h_Lb, d_Lb, sizeof(float)*NPATH,
                  cudaMemcpyDeviceToHost, stream_app) );
