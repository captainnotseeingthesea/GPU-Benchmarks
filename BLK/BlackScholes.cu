...
#else
const int  NUM_ITERATIONS = 3; // 512; 
#endif
...
//int main(int argc, char **argv){
	...
	h_CallResultCPU = (float *)malloc(OPT_SZ);
	 h_PutResultCPU  = (float *)malloc(OPT_SZ);
	  ...
	  h_OptionYears   = (float *)malloc(OPT_SZ);
        ...
        cutilSafeCall( cudaMemcpyAsync(d_StockPrice,  h_StockPrice,   OPT_SZ,
        cudaMemcpyHostToDevice, stream_app));
        cutilSafeCall( cudaMemcpyAsync(d_OptionStrike, h_OptionStrike,  OPT_SZ,
        cudaMemcpyHostToDevice, stream_app));
        cutilSafeCall( cudaMemcpyAsync(d_OptionYears,  h_OptionYears,   OPT_SZ,
        cudaMemcpyHostToDevice, stream_app));
	...
	printf("Executing Black-Scholes GPU kernel (%i iterations)...\n", NUM_ITERATIONS);
        ...
        for(i = 0; i < NUM_ITERATIONS; i++){
            BlackScholesGPU<<<480, 128, 0, stream_app>>>(
                d_CallResult,
                d_PutResult,
                d_StockPrice,
                d_OptionStrike,
    ...
    printf("Reading back GPU results...\n");
        ...
        cutilSafeCall( cudaMemcpyAsync(h_CallResultGPU, d_CallResult, OPT_SZ,
        cudaMemcpyDeviceToHost, stream_app));
        cutilSafeCall( cudaMemcpyAsync(h_PutResultGPU,  d_PutResult,  OPT_SZ,
        cudaMemcpyDeviceToHost, stream_app));
