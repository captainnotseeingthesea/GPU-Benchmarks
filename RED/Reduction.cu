...
template <class T>
void RunTest(string testName, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag);
...
int main_RED(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
    ...
    RunTest<float>("Reduction", stream_app,mutexapp, flag);
               ...
               (deviceProp.major >= 2))
    {
        ...
        RunTest<double>("Reduction-DP", stream_app, mutexapp, flag) ;
    } else {
...
void RunTest(string testName, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
    ...
    for (int k=0; k<passes; k++)
    {
        ...
        CUDA_SAFE_CALL(cudaMemcpyAsync(d_idata, h_idata, size*sizeof(T),
       ...
       // cudaEventSynchronize(stop);
        ...
        // Copy back to host
        //cudaEventRecord(start, 0);
        CUDA_SAFE_CALL(cudaMemcpyAsync(h_odata, d_odata,
                num_blocks*sizeof(T), cudaMemcpyDeviceToHost, stream_app));
       // cudaEventRecord(stop, 0);
        //cudaEventSynchronize(stop);
        float output_time;
