...
/////////////////////////////////////////////////////////////////////////////////////////
int
main_CN(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
...
void InitHostMem(double *Layer1_Weights_CPU, double *Layer2_Weights_CPU, double *Layer3_Weights_CPU, double *Layer4_Weights_CPU, double *Layer5_Weights_CPU)
{
	...
	FILE * pFile1 = fopen ("data_CN/conv1.txt","rb");
	if (pFile1 != NULL)
	{
		//printf("File Opened\n");
		char s[300000] = "";
	...
	//Layer 2 Weights
	FILE * pFile2 = fopen ("data_CN/conv2.txt","rb");
	if (pFile2 != NULL)
	{
	...
	//Layer 3 Weights
	FILE * pFile3 = fopen ("data_CN/conv3.txt","rb");
	if (pFile3 != NULL)
	{
	...
	//Layer 4 Weights
	FILE * pFile4 = fopen ("data_CN/ip1.txt","rb");
	if (pFile4 != NULL)
	{
	...
	//Layer 5 Weights
	FILE * pFile5 = fopen ("data_CN/ip2.txt","rb");
	if (pFile5 != NULL)
	{
...
{
	FILE * pFile1 = fopen ("data_CN/speed-limit-35.txt","rb");
	if (pFile1 != NULL)
	{
		//printf("File Opened\n");
		char s[300000] = "";
...
static void NeuralNetwork(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	//Allocation of host memory for input data_CN
	...
	//Allocation of device memory for input data_CN
	...
	cudaEventRecord(start, stream_app);
	err = cudaMalloc((void**) &Layer1_Features, 32*32*32* NUM * sizeof(double));
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to allocate device data_CN (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	...
	cudaMemcpyAsync(Layer1_Weights_GPU,Layer1_Weights_CPU, sizeof(double)*2400*NUM, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(Data_Layer_GPU_R,Data_Layer_CPU_R, 32*32* NUM * sizeof(int), cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(Data_Layer_GPU_G,Data_Layer_CPU_G, 32*32* NUM * sizeof(int), cudaMemcpyHostToDevice, stream_app);
	err = cudaMemcpyAsync(Data_Layer_GPU_B,Data_Layer_CPU_B, 32*32* NUM * sizeof(int), cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_CN (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	...
	//Execute First Layer
	ExecuteFirstLayer<<<n_blocks,n_threads, 0, stream_app>>>(Layer1_Weights_GPU, Data_Layer_GPU_R, Data_Layer_GPU_G, Data_Layer_GPU_B, Layer1_Features);
	...
	pooling1<<<n_blocks,n_threads, 0, stream_app>>>(Layer1_Features, Pool_Layer_Features, 32, 16, 16, 5, 2, 32, 32);
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	...
	cudaMemcpyAsync(Layer2_Weights_GPU,Layer2_Weights_CPU, sizeof(double)*5*5*32*32*NUM, cudaMemcpyHostToDevice, stream_app);
	...
	ExecuteSecondLayer<<<n_blocks,n_threads, 0, stream_app>>>(Layer2_Weights_GPU, Layer2_Features, Pool_Layer_Features);
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	...
	pooling2<<<n_blocks,n_threads, 0, stream_app>>>(Layer2_Features, Pool2_Layer_Features, 32, 8, 8, 5, 2, 16, 16);
	cudaThreadSynchronize();
	err = cudaGetLastError();
        if (err != cudaSuccess)
        {
	...
	cudaMemcpyAsync(Layer3_Weights_GPU,Layer3_Weights_CPU, sizeof(double)*5*5*32*64*NUM, cudaMemcpyHostToDevice, stream_app);
	...
	//Execute Third Layer
	ExecuteThirdLayer<<<n_blocks,n_threads, 0, stream_app>>>(Layer3_Weights_GPU, Layer3_Features, Pool2_Layer_Features);
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	...
	pooling3<<<n_blocks,n_threads, 0, stream_app>>>(Layer3_Features, Pool3_Layer_Features, 64, 4, 4, 5, 2, 8, 8);
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	...
	cudaMemcpyAsync(Layer4_Weights_GPU,Layer4_Weights_CPU, sizeof(double)*64*4*4*64*NUM, cudaMemcpyHostToDevice, stream_app);
	//Execute Fourth Layer
	ExecuteFourthLayer<<<1,64, 0, stream_app>>>(Layer4_Weights_GPU, Layer4_Features, Pool3_Layer_Features);	
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	...
	cudaMemcpyAsync(Layer5_Weights_GPU,Layer5_Weights_CPU, sizeof(double)*64*9*NUM, cudaMemcpyHostToDevice, stream_app);
	//Execute Fifth Layer
	ExecuteFifthLayer<<<1,32, 0, stream_app>>>(Layer5_Weights_GPU, Layer5_Features, Layer4_Features);	
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	...
	cudaMemcpyAsync(Layer5_output_CPU, Layer5_Features, 9* NUM * sizeof(double), cudaMemcpyDeviceToHost, stream_app);
	cudaEventRecord(stop, stream_app);
