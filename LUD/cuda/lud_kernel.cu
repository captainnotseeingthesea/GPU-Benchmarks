...
void lud_cuda(float *m, int matrix_dim, cudaStream_t stream_app)
{
  int i=0;
  ...
  for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
      lud_diagonal<<<1, BLOCK_SIZE,0, stream_app>>>(m, matrix_dim, i);
      lud_perimeter<<<(matrix_dim-i)/BLOCK_SIZE-1, BLOCK_SIZE*2,0, stream_app>>>(m, matrix_dim, i);
      dim3 dimGrid((matrix_dim-i)/BLOCK_SIZE-1, (matrix_dim-i)/BLOCK_SIZE-1);
      lud_internal<<<dimGrid, dimBlock,0, stream_app>>>(m, matrix_dim, i); 
  }
  lud_diagonal<<<1,BLOCK_SIZE,0,stream_app>>>(m, matrix_dim, i);
}
