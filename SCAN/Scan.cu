...
int main_scan(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
    ...
    RunTest<float, float4>(stream_app, mutexapp,flag);
               ...
               (deviceProp.major >= 2))
    {
        ...
        RunTest<double, double4>(stream_app, mutexapp,flag);
    } else {
...
void RunTest(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
   ...
   // CUDA_SAFE_CALL(cudaEventCreate(&stop));
    ...
    CUDA_SAFE_CALL(cudaMemcpyAsync(d_idata, h_idata, bytes, cudaMemcpyHostToDevice,stream_app));
    ...
    for (int k = 0; k < passes; k++)
    {
        ...
        for (int j = 0; j < iters; j++)
        {
            ...
            reduce<T,256><<<num_blocks, num_threads, smem_size,stream_app>>>
                (d_idata, d_block_sums, size);

            // Next, a top-level exclusive scan is performed on the array
            // of block sums
            scan_single_block<T,256><<<1, num_threads, smem_size*2,stream_app>>>
            ...
            bottom_scan<T, vecT><<<num_blocks, num_threads, 2*smem_size,stream_app>>>
        ...
        CUDA_SAFE_CALL(cudaMemcpyAsync(h_odata, d_odata, bytes,
