...
extern "C"
int setup(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag);									/* function prototype */
...
extern "C"
void allocateMemory(int npoints, int nfeatures, int nclusters, float **features, cudaStream_t stream_app)
{	
	...
	cudaMemcpyAsync(feature_flipped_d, features[0], npoints*nfeatures*sizeof(float), cudaMemcpyHostToDevice, stream_app);
...
int
main_KMEANS(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag) 
{
	// make sure we're running on the big card
    cudaSetDevice(1);
	// as done in the CUDA start/help document provided
	setup(stream_app, mutexapp,flag);    
}
		   ...
		   )
{
	...
	cudcudaMemcpyAsyncaMemcpy(membership_d, membership_new, npoints*sizeof(int), cudaMemcpyHostToDevice, stream_app);

	/* copy clusters (host to device) */
	cudaMemcpyAsync(clusters_d, clusters[0], nclusters*nfeatures*sizeof(float), cudaMemcpyHostToDevice, stream_app);
