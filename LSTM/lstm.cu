...
static void NeuralNetwork(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag);
...
void InitHostMem(double *w_i, double *u_i, double *b_i, double *w_f, double *u_f, double *b_f, double *w_c, double *u_c, double *b_c, double *w_o, double *u_o, double *b_o, double *weight, double *bias)
{
	...
	FILE * pFile1 = fopen ("data_LSTM/w_i.txt","rb");
	if (pFile1 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile2 = fopen ("data_LSTM/b_i.txt","rb");
	if (pFile2 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile3 = fopen ("data_LSTM/w_f.txt","rb");
	if (pFile3 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile4 = fopen ("data_LSTM/b_f.txt","rb");
	if (pFile4 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile5 = fopen ("data_LSTM/w_c.txt","rb");
	if (pFile5 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile6 = fopen ("data_LSTM/b_c.txt","rb");
	if (pFile6 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile7 = fopen ("data_LSTM/w_o.txt","rb");
	if (pFile7 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile8 = fopen ("data_LSTM/b_o.txt","rb");
	if (pFile8 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile9 = fopen ("data_LSTM/W.txt","rb");
	if (pFile9 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	*bias = weight[100];	
	...
	FILE * pFile10 = fopen ("data_LSTM/u_o.txt","rb");
	if (pFile10 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile11 = fopen ("data_LSTM/u_i.txt","rb");
	if (pFile11 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile12 = fopen ("data_LSTM/u_c.txt","rb");
	if (pFile12 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
	...
	FILE * pFile13 = fopen ("data_LSTM/u_f.txt","rb");
	if (pFile13 != NULL)
	{
		//printf("File Opened\n");
		char s[1000000] = "";
...
static void NeuralNetwork(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to allocate device data_LSTM (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	double *u_i_device;
	...
	//Start Memory Copy
	err = cudaMemcpyAsync(w_i_device, w_i, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 1(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(u_i_device, u_i, sizeof(double)*10000*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 2(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(b_i_device, b_i, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 3(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(w_f_device, w_f, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 4(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(u_f_device, u_f, sizeof(double)*10000*NUM, cudaMemcpyHostToDevice,stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 5(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(b_f_device, b_f, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 6(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(w_c_device, w_c, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 7(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(u_c_device, u_c, sizeof(double)*10000*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 8(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(b_c_device, b_c, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM 9(error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(w_o_device, w_o, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(u_o_device, u_o, sizeof(double)*10000*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(b_o_device, b_o, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(weights_device, weights, sizeof(double)*100*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device data_LSTM (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMemcpyAsync(bias_device, bias, sizeof(double)*1*NUM, cudaMemcpyHostToDevice, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy device bias data_LSTM (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	//printf("Memcpy completed\n");
	dim3 n_threads(100,1,1);
	dim3 n_blocks(1,1,1);
	ExecuteLSTM<<<n_blocks,n_threads, 0, stream_app>>>(w_i_device, u_i_device, b_i_device, w_f_device, u_f_device, b_f_device, w_c_device, u_c_device, b_c_device, w_o_device, u_o_device, b_o_device, weights_device, bias_device, LSTM_results);
	pthread_mutex_unlock (mutexapp);
	if(flag)
		cudaStreamSynchronize(stream_app);
	else
		cudaThreadSynchronize();
	
	err = cudaMemcpyAsync(result, LSTM_results, sizeof(double)*2*NUM, cudaMemcpyDeviceToHost, stream_app);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "Failed to copy result data_LSTM (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
