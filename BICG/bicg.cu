...
static void compareResults(DATA_TYPE* s, DATA_TYPE* s_outputFromGpu, DATA_TYPE* q, DATA_TYPE* q_outputFromGpu)
{
	int i,fail;
	fail = 0;
...
void bicgCuda(DATA_TYPE* A, DATA_TYPE* r, DATA_TYPE* s, DATA_TYPE* p, DATA_TYPE* q,
			DATA_TYPE* s_outputFromGpu, DATA_TYPE* q_outputFromGpu, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	cudaMemcpyAsync(A_gpu, A, sizeof(DATA_TYPE) * NX * NY, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(r_gpu, r, sizeof(DATA_TYPE) * NX, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(s_gpu, s, sizeof(DATA_TYPE) * NY, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(p_gpu, p, sizeof(DATA_TYPE) * NY, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(q_gpu, q, sizeof(DATA_TYPE) * NX, cudaMemcpyHostToDevice, stream_app);
	...
	cudaMemcpyAsync(s_outputFromGpu, s_gpu, sizeof(DATA_TYPE) * NY, cudaMemcpyDeviceToHost, stream_app);
	cudaMemcpyAsync(q_outputFromGpu, q_gpu, sizeof(DATA_TYPE) * NX, cudaMemcpyDeviceToHost, stream_app);
...
int main_BICG(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	bicgCuda(A, r, s, p, q, s_outputFromGpu, q_outputFromGpu, stream_app, mutexapp, flag);
