...
static void compareResults(DATA_TYPE* B, DATA_TYPE* B_outputFromGpu)
{
	int i, j, k, fail;
	fail = 0;
...
void convolution3DCuda(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* B_outputFromGpu, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	cudaMemcpyAsync(A_gpu, A, sizeof(DATA_TYPE) * NI * NJ * NK, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(B_gpu, B, sizeof(DATA_TYPE) * NI * NJ * NK, cudaMemcpyHostToDevice, stream_app);
	...
	cudaMemcpyAsync(B_outputFromGpu, B_gpu, sizeof(DATA_TYPE) * NI * NJ * NK, cudaMemcpyDeviceToHost, stream_app);
...
int main_3DCONV(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	convolution3DCuda(A, B, B_outputFromGpu, stream_app, mutexapp, flag);
