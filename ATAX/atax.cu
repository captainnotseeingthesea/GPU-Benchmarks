...
static void compareResults(DATA_TYPE *z, DATA_TYPE *z_outputFromGpu)
{
	int i, fail;
	fail = 0;
...
void ataxGpu(DATA_TYPE* A, DATA_TYPE* x, DATA_TYPE* y, DATA_TYPE* tmp, DATA_TYPE* y_outputFromGpu, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	cudaMemcpyAsync(x_gpu, x, sizeof(DATA_TYPE) * NY, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(y_gpu, y, sizeof(DATA_TYPE) * NY, cudaMemcpyHostToDevice, stream_app);
	cudaMemcpyAsync(tmp_gpu, tmp, sizeof(DATA_TYPE) * NX, cudaMemcpyHostToDevice, stream_app);
	...
	cudaMemcpyAsync(y_outputFromGpu, y_gpu, sizeof(DATA_TYPE) * NX, cudaMemcpyDeviceToHost, stream_app);
...
int main_ATAX(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
	...
	GPU_argv_init();
	ataxGpu(A, x, y, tmp, y_outputFromGpu, stream_app, mutexapp, flag);
