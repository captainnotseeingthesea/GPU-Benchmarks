...
int main_MUM(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
   ...
   Reference ref;
   if ((err = createReference("MUM/data/NC_003997.20k.fna", &ref)))
   {
   ...
   QuerySet queries;
   if ((err = createQuerySet("MUM/data/NC_003997_q25bp.50k.fna", &queries)))
   {
	  printStringForError(err);
	  exit(err);
   }
   ...
   if ((err = matchQueries(&ctx,stream_app,mutexapp,flag)))
   {
	  printStringForError(err);
	  exit(err);
   }   
