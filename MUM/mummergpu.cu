...
void loadQueries(MatchContext* ctx,cudaStream_t stream_app)
{
   //unsigned int toboardtimer = 0;
   //createTimer(&toboardtimer);
   //startTimer(toboardtimer);
...
void transferResultsFromDevice(MatchContext* ctx,cudaStream_t stream_app)
{
   if (!ctx->on_cpu)
   {
...
int matchSubset(MatchContext* ctx, 
				...
				ReferencePage pages[],
				unsigned int num_pages,cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
   
   loadQueries(ctx, stream_app);
   ...
   { 
	  ...
	  else
	  {
		 ...
		 if (alignRC)
		 {
			mummergpuRCKernel<<< dimGrid, dimBlock, 0 ,stream_app>>>(ctx->results.d_match_coords,
		 ...
		 {
			mummergpuKernel<<< dimGrid, dimBlock, 0 , stream_app>>>(ctx->results.d_match_coords,
													 ctx->queries->d_tex_array,
													 ctx->queries->d_addrs_tex_array,
													 ctx->queries->d_lengths_array,
													 numQueries,
	 ...
	 // fprintf(stderr,"kernel time= %f\n", ktime);
	  ...
	  transferResultsFromDevice(ctx, stream_app);
...
{	   
   ...
   {
	  pthread_mutex_unlock (mutexapp);
	  matchSubset(ctx, 0, pages, num_reference_pages,stream_app,mutexapp,flag);
